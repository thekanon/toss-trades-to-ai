...
export function parseLine(line) {
  ...
  if (p.length < 4) return null;   // guard empty/broken lines
...
export function parseText(raw) {
  // 전처리: 헤더/꼬리글 제거 + 연속 개행 축소
  const cleaned = raw
    // 날짜형식(202x.yy.dd)으로 시작하지 않는 라인 제거
    .replace(/^(?!202\d\.\d{2}\.\d{2}).*$/gm, "")
    // 두 줄 이상 연속 개행을 하나로 축소
    .replace(/\n{2,}/g, "\n")
    .trim();
...
export function compact(records, target = "date", period = "day") {
  ...
  for (const r of records) {
    ...
    if (["구매", "매수", "매도"].includes(kind)) {
      ...
      if (price && amount && qty && price > 10_000 && amount < price && Math.abs(price - amount * qty) <= 2) {
        [price, amount] = [amount, price]; // swap if inverted
      }
  ...
  /* ─ day mode (no aggregation) ─ */
  if (period === "day") {
    for (const t of trades) {
      const k = target === "symbol" ? t.symbol : t.date;
      const row = target === "symbol"
        ? [t.date, t.qty, Math.round(t.price)]
  ...
  for (const [k1, inner] of Object.entries(agg)) {
    out[k1] = Object.entries(inner).map(([k2, v]) => [
      k2,
      +v.q.toFixed(6),
      Math.round(v.s / v.q),
      Math.round(v.s),
